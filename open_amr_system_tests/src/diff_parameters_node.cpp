/**
 * @file diff_parameters_node.cpp
 * @brief This ROS 2 node demonstrates how to declare parameters and publish velocity messages.
 *
 * This program implements a ROS 2 node that declares parameters for a diff wheel robot 
 ...
 *
 * Publishing Topics:
 *   /diff_drive_controller/cmd_vel (geometry_msgs/TwistStamped): Velocity command for the robot
 ...
 */
class DiffParameters : public rclcpp::Node
{
public:
  /**
   * @brief Constructor for the diffParameters node.
   *
   ...
   */
  DiffParameters() : Node("diff_parameters_node")
  {
    // Describe parameters
    ...
    this->declare_parameter("robot_name", "Mohannad Bot", robot_name_descriptor);
    ...
    param_callback_handle_ = this->add_on_set_parameters_callback(
      std::bind(&DiffParameters::parameter_change_callback, this, std::placeholders::_1));

    // Create a publisher for the /diff_drive_controller/cmd_vel topic
    velocity_publisher_ = this->create_publisher<geometry_msgs::msg::TwistStamped>("/diff_drive_controller/cmd_vel", 10);

    // Create a timer that will call the publish_velocity function every 0.5 seconds (2 Hz)
    timer_ = this->create_wall_timer(std::chrono::milliseconds(500), std::bind(&DiffParameters::publish_velocity, this));
  }
...
private:
  ...
  /**
   * @brief Publishes velocity messages to the /diff_drive_controller/cmd_vel topic.
   *
 ...
 *
 * Initializes ROS 2, creates an instance of the diffParameters node,
...
int main(int argc, char ** argv)
{
  ...
  auto diff_parameters_node = std::make_shared<DiffParameters>();
  rclcpp::spin(diff_parameters_node);
  rclcpp::shutdown();
  return 0;
}
