...
(set by OpenAMR tools/setup/openamr_env.sh) unless `graph`/`graph_nodes` are given. Home of amr_i = charge_i.
"""
import os
...
def spawn(context):
    ...
    actions = [Node(package='open_amr_swarm', executable='swarm_agent', namespace=f'amr_{i}', name='swarm_agent',
                    output='screen',
                    parameters=[{'robot_id': f'amr_{i}', 'graph': graph, 'home_node': f'charge_{i}',
                                 'use_sim_time': True}],
